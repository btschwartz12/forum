...
func NewRepo(logger *zap.SugaredLogger, varDir string) (*Repo, error) {
	...
	if err := os.MkdirAll(filepath.Join(varDir, postUploadDir), 0755); err != nil {
		return nil, fmt.Errorf("error creating post upload dir: %w", err)
	}
